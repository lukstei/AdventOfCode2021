...
fn solution1<'a>(input: &str) -> Result<String> {
    ...
    let ys = xs.iter()
        .map(|l| {
            let (v1) = (
                l[1].as_str().chars().map(|x|x.to_digit(10).unwrap() as i32).collect_vec()
            );
    ...
    impl<'a> Pos<'a> {
        fn successors(&self) -> Vec<(Pos<'a>, usize)> {
            ...
            vec![(x-1, y), (x, y-1), (x+1, y), (x, y+1)]
                .into_iter().filter(|(x,y)|*x>=0&&*y>=0&&(*x< self.3[0].len() as i32)&&(*y< self.3.len() as i32))
                .map(|(x,y)| {
    ...
    let GOAL: Pos = Pos((ys[0].len() - 1) as i32, ys.len() as i32, ys[ys.len()-1][ys[0].len()-1], &ys);
    let result = dijkstra(&Pos(0, 0, ys[0][0], &ys), |p: &Pos|p.successors(), |p| p.0== (ys[0].len() - 1) as i32 && p.1== (ys.len() - 1) as i32);
...
fn solution2<'a>(input: &str) -> Result<String> {
    ...
    let mut nys = (0..y_len*5).map(|y|(0..x_len*5).map(|x|0).collect_vec()).collect_vec();
