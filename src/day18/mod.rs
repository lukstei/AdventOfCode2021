...
impl Node {
    fn magnitude(&self) -> u32 {
        match self {
            Num(x) => *x,
            Node::Leaf(s) => {
                s.l.magnitude() * 3 + s.r.magnitude() * 2
            }
        }
    }
...
fn solution2(input: &str) -> Result<String> {
    ...
    let max = trees.iter().tuple_combinations().map(|(x1, x2)| Node::add(x1.clone(), x2.clone()).magnitude().max(Node::add(x2.clone(), x1.clone()).magnitude())).max().unwrap();
...
mod tests {
    ...
    use crate::day18::{Node, solution1, solution2, Tree};
...
[5,5]".lines()
        ...
        assert_eq!("[[[[3,0],[5,3]],[4,4]],[5,5]]", format!("{}", Node::add_all(trees)));
    }
...
[[[[4,2],2],6],[8,7]]".lines()
        ...
        assert_eq!("[[[[8,7],[7,7]],[[8,6],[7,7]]],[[[0,7],[6,6]],[8,7]]]", format!("{}", Node::add_all(trees)));
    }
    ...
    fn test_parse() {
        ...
        assert_eq!("[[[[1,3],[5,3]],[[1,3],[8,7]]],[[[4,9],[6,9]],[[8,2],[7,3]]]]", solution1(indoc!("[[[[1,3],[5,3]],[[1,3],[8,7]]],[[[4,9],[6,9]],[[8,2],[7,3]]]]")).unwrap());
    }

    #[test]
    fn test_magnitude() {
        let mut tree = Node::Leaf(Box::new(Tree::parse_tree("[[[[8,7],[7,7]],[[8,6],[7,7]]],[[[0,7],[6,6]],[8,7]]]").1));
    ...
    #[test]
    fn test_reduce() {
        let mut tree = Node::Leaf(Box::new(Tree::parse_tree("[[[[[4,3],4],4],[7,[[8,4],9]]],[1,1]]").1));
        tree.reduce();
    ...
    #[test]
    fn test_split() {
        let mut tree = Node::Leaf(Box::new(Tree::parse_tree("[[[[0,7],4],[15,[0,13]]],[1,1]]").1));
        tree.split();
    ...
    #[test]
    fn test_explode4() {
        let mut tree = Node::Leaf(Box::new(Tree::parse_tree("[[3,[2,[1,[7,3]]]],[6,[5,[4,[3,2]]]]]").1));
        tree.explode(0);
    ...
    #[test]
    fn test_explode5() {
        let mut tree = Node::Leaf(Box::new(Tree::parse_tree("[[3,[2,[8,0]]],[9,[5,[4,[3,2]]]]]").1));
        tree.explode(0);
    ...
    fn test_part1() {
        assert_eq!("4140",
                   solution1(indoc!("[[[0,[5,8]],[[1,7],[9,6]]],[[4,[1,2]],[[1,4],2]]]
[[[5,[2,8]],4],[5,[[9,9],0]]]
[6,[[[6,2],[5,6]],[[7,6],[4,7]]]]
[[[6,[0,7]],[0,9]],[4,[9,[9,0]]]]
[[[7,[6,4]],[3,[1,3]]],[[[5,5],1],9]]
