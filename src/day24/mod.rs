...
pub fn solution1_s(input: &str) -> Result<String> {
    let sub_programs = vec![
        "inp w
mul x 0
add x z
...
add z y",
    ...
    let mut target_z_states: HashSet<Word> = Default::default();
    target_z_states.insert(0);
    let mut valid_states: Vec<HashSet<(Word, Word, Word)>> =
        (0..14).map(|x| Default::default()).collect_vec();

    let mut ranges = (0..14).map(|x| None).collect_vec();
    ranges[13] = Some(13..=21);
    ranges[12] = Some(0..=570);
    ranges[11] = Some(0..=14841);
    ranges[10] = Some(0..=570);
    ranges[9] = Some(0..=14841);
    ranges[8] = Some(0..=570);
    ranges[7] = Some(0..=14832);
    ranges[6] = Some(0..=570);
    ranges[5] = Some(0..=14833);
    ranges[4] = Some(0..=385681);
    ranges[3] = Some(0..=14833);
    ranges[2] = Some(0..=570);
    ranges[1] = Some(0..=21);
    ranges[0] = Some(0..=0);

    for (i, x) in sub_programs.iter().enumerate().rev() {
        let mut valid_z_states: HashSet<Word> = Default::default();
        ...
        for input in 1..=9 {
            let range = ranges[i].clone().unwrap_or(-10000..=1000000);
            ...
            for z in range {
                ...
                match state.exec_until_end(&parse(*x)) {
                    None => {}
                    Some(z_out) => {
                        if target_z_states.contains(&z_out) {
                            //println!("At i={}: Reached target z state {} with z={} and input={}", i, z_out, z, input);
                            valid_z_states.insert(z);
                            valid_states[i].insert((z_out, z, input));
    ...
    println!(
        "States 2:{}",
        valid_states[2].iter().map(|x| x.1.to_string()).join(",")
    );
    println!(
        "States len:{}",
        valid_states.iter().map(|x| x.len().to_string()).join(",")
    );

    let inpu = valid_states
        .iter()
        .map(|x| {
            let mut map: HashMap<Word, HashMap<Word, Word>> = Default::default();
            for (z_out, z, input) in x.iter() {
                if !map.contains_key(input) {
                    map.insert(*input, Default::default());
                }
                let x1 = map.get_mut(input).unwrap();
                x1.insert(*z, *z_out);
            }
            map
        })
    ...
    let result = format!("{}", find_lowest_reachable_number(&inpu, 0, 0).unwrap());
    println!("{}", result);
    Ok(result)
}

fn find_lowest_reachable_number(
    ...
    (0..=9)
        .filter_map(|num| match x[i].get(&num) {
            ...
            Some(zs) => match zs.get(&z) {
                None => None,
                Some(next_z) => find_lowest_reachable_number(x, i + 1, *next_z)
...
fn solution1_r(input: &str) -> Result<String> {
    let sub_programs = vec![
        "inp w
mul x 0
add x z
...
add z y",
    ...
    ba.exec(&parse(sub_programs[0]));
    println!("1={}, x={}, y={}, w={}", ba.z, ba.x, ba.y, ba.w);

    let z_state = Expr::Val(0);
    ...
    for (i, x) in sub_programs.iter().enumerate() {
        ...
        ba.exec(&parse(*x));
        println!("{}={}", i, ba.z);
        //println!("{}={}", i, Regex::new("inp_([0-9]+)").unwrap().replace_all(&format!("{}", ba.z).replace("z", "$E11"), "F$$10"));
    }
...
mod tests {
    use crate::day24::{
        parse, solution1, solution1_r, solution1_s, solution2, BackAnalyzer, Expr, State,
    };
...
add z y",
    ...
    #[test]
    fn run_solution1_s() {
        run_solution(INPUT, solution1_s).unwrap()
    }

    #[test]
    fn run_solution1_r() {
        run_solution(INPUT, solution1_r).unwrap()
    }
