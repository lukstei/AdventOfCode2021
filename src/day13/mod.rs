...
fn fold_x(along_x: usize, marks: &mut HashSet<(usize, usize)>, max_x: usize, max_y: usize) {
    (0..=max_y).for_each(|y| { marks.remove(&(along_x, y)); });
...
fn fold_y(along_y: usize, marks: &mut HashSet<(usize, usize)>, max_x: usize, max_y: usize) {
    (0..=max_x).for_each(|x| { marks.remove(&(x, along_y)); });
...
9,0"), vec![("y", 7)]).unwrap());
    ...
    fn run_solution1() {
        ...
        println!("Solution with input: {}", solution1(input.trim(), vec![("x", 655)]).unwrap());
    }
    ...
    // PART 2
        ...
        fn run_solution2(){
            ...
            println!("Solution with input: {}", solution2(input.trim(), vec![("x",655),("y",447),("x",327),("y",223),("x",163),("y",111),("x",81),("y",55),("x",40),("y",27),("y",13),("y",6)]).unwrap());
        }
}
